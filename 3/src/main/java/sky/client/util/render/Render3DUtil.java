package sky.client.util.render;
...
public class Render3DUtil implements IMinecraft {
    ...
    private static BufferBuilder FLAT_BUILDER;

    public static void startFlatRender(Identifier texture) {
        ...
        FLAT_BUILDER = Tessellator.getInstance().begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_TEXTURE_COLOR);
    }

    public static void flatImage(MatrixStack stack, float x, float y, float z, float width, float height, int color) {
        if (FLAT_BUILDER == null) return;

        Matrix4f matrix = stack.peek().getPositionMatrix();

        FLAT_BUILDER.vertex(matrix, x, y + height, z).texture(0, 1).color(color);
        FLAT_BUILDER.vertex(matrix, x + width, y + height, z).texture(1, 1).color(color);
        FLAT_BUILDER.vertex(matrix, x + width, y, z).texture(1, 0).color(color);
        FLAT_BUILDER.vertex(matrix, x, y, z).texture(0, 0).color(color);
    }

    public static void endFlatRender() {
        if (FLAT_BUILDER != null) {
            BuiltBuffer built = FLAT_BUILDER.endNullable();
            if (built != null) {
