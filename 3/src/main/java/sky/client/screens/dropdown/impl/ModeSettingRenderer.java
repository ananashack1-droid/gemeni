...
public class ModeSettingRenderer implements SettingRenderer<ModeSetting>, IMinecraft {
    ...
    public void render(DrawContext ctx, ModeSetting setting, int x, int y, int width, int height) {
        ...
        for (String mode : setting.getModes()) {
            ...
            hoverProgress.put(mode, mode.equals(setting.get()) ? 1f : hoverProgress.getOrDefault(mode, 0f) + ((RenderUtil.isInRegion(mc.mouse.getX() / mc.getWindow().getScaleFactor(), mc.mouse.getY() / mc.getWindow().getScaleFactor(), currentX, currentY, (int) FontUtils.durman[13].getWidth(mode) + 6, 12) ? 1f : 0f) - hoverProgress.getOrDefault(mode, 0f)) * 0.08f);
            RenderUtil.drawRoundedRect(ctx.getMatrices(), currentX, currentY, (int) FontUtils.durman[13].getWidth(mode) + 6, 12, 1, mode.equals(setting.get()) ? Manager.STYLE_MANAGER.getFirstColor() : ColorUtil.interpolateColor(new Color(30, 30, 30, 180).getRGB(), new Color(30, 30, 30, 180).getRGB(), hoverProgress.get(mode)));
    ...
    public boolean mouseClicked(ModeSetting setting, double mouseX, double mouseY, int button, int x, int y, int width, int height) {
        ...
        for (String mode : setting.getModes()) {
            ...
            if (RenderUtil.isInRegion((int) mouseX, (int) mouseY, currentX, currentY, (int) FontUtils.durman[13].getWidth(mode) + 6, 12)) {
                setting.set(mode);
                return true;
            }
